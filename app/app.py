...
def build_server_conf(server_ip, port, proto, pki_dir,
                      target_net: tuple[str, str] | None = None) -> str:
    ...
    if target_net:
        ...
        routing = (
            f'push "route {net_addr} {net_mask}"'
        )
    else:
        routing = (
...
def generate_vpn(jid: str, server_ip: str, port: int, proto: str,
                 ...
                 target_net: tuple[str, str] | None = None) -> None:
    status = JOBS[jid]
    ...
    try:
        ...
        server_conf = build_server_conf(server_ip, port, proto, pki_dir, target_net)
...
def create():
    ...
    threading.Thread(
        target=generate_vpn,
        args=(jid, server_ip, port, proto, client, cert_days, target_net),
        daemon=True,
    ).start()
