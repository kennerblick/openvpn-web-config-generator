...
def build_server_conf(server_ip, port, proto, pki_dir) -> str:
...
def generate_vpn(jid: str, server_ip: str, port: int, proto: str,
    ...
    try:
        ...
        server_conf = build_server_conf(server_ip, port, proto, pki_dir)
...
def create():
    ...
    threading.Thread(
        target=generate_vpn,
        args=(jid, server_ip, port, proto, client, cert_days),
        daemon=True,
    ).start()
